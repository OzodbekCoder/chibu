<div class="px-4 py-4 space-y-4">
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            <div class="mt-1 text-3xl font-bold text-slate-800">{{ $activeCount }}</div>
            <div class="text-xs text-slate-500 mt-1">Hozir kuzatilmoqda</div>
        </div>
        <div class="bg-white rounded-2xl border border-slate-200 p-4">
            <div class="text-[11px] text-slate-400 font-medium uppercase tracking-wide">Bugun qabul</div>
            <div class="mt-1 text-3xl font-bold text-emerald-600">{{ $deliveredToday }}</div>
            <div class="text-xs text-slate-500 mt-1">Yetkazildi</div>
        </div>
    </div>
    ...
    @endphp
    <div class="bg-white rounded-2xl border border-slate-200 p-4">
        <div class="text-sm font-semibold mb-3">Holat bo'yicha</div>
        <div class="space-y-2">
            @foreach ($statusInfo as $key => [$label, $color])
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 text-sm text-slate-700">{{ $label }}</div>
                    <div class="font-semibold text-sm">{{ $cnt }}</div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-3 space-y-2">
        <div class="flex gap-1.5 overflow-x-auto -mx-1 px-1">
            @foreach (['today'=>'📅 Bugun','week'=>'📆 Hafta','month'=>'🗓 Oy','year'=>'📊 Yil','custom'=>'🎯 Oraliq'] as $key=>$label)
                <button wire:click="setRange('{{ $key }}')"
                    class="px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap shrink-0 {{ $range === $key ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700' }}">
                    {{ $label }}
                </button>
            @endforeach
        </div>
        @if ($range === 'custom')
            <div class="grid grid-cols-2 gap-2">
                <input wire:model="from" type="date" class="rounded-lg border-slate-300 px-2 py-2 text-sm border">
                <input wire:model="to"   type="date" class="rounded-lg border-slate-300 px-2 py-2 text-sm border">
            </div>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white rounded-2xl border border-slate-200 p-4">
            ...
            <div class="mt-1 text-3xl font-bold text-slate-800">{{ number_format($rangeCount) }}</div>
            <div class="text-xs text-slate-500 mt-1">Ta qo'shildi</div>
        </div>
        <div class="bg-white rounded-2xl border border-slate-200 p-4">
            <div class="text-[11px] text-slate-400 font-medium uppercase tracking-wide">Tovar narxi</div>
            <div class="mt-1 text-3xl font-bold text-slate-800">{{ number_format((float) $rangeYuan, 0) }}</div>
            <div class="text-xs text-slate-500 mt-1">¥ jami</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-3">
        ...
        <a href="{{ route('app.shipments.search') }}" wire:navigate
           class="bg-white rounded-2xl p-3 border border-slate-200 flex flex-col items-center gap-1">
            <div class="text-2xl">🔍</div>
            <div class="text-xs font-medium text-slate-700">Qidirish</div>
        </a>
        <a href="{{ route('app.reports.index') }}" wire:navigate
           class="bg-white rounded-2xl p-3 border border-slate-200 flex flex-col items-center gap-1">
            <div class="text-2xl">📊</div>
            <div class="text-xs font-medium text-slate-700">Hisobot</div>
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200">
        <div class="px-4 py-3 flex items-center justify-between border-b border-slate-100">
            <div class="font-semibold text-sm">Faol yuklar</div>
        ...
        <div class="divide-y divide-slate-100">
            ...
            @php
                $statusColors = [
                    ...
                    'CHINA_WAREHOUSE' => 'bg-amber-100 text-amber-700',
            ...
            <div class="px-4 py-3">
                <div class="flex items-start justify-between gap-2">
                    <div class="min-w-0">
                        ...
                        <div class="font-medium text-sm mt-0.5 truncate">{{ $s->client?->name ?? '—' }}</div>
                    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-8 text-center">
        ...
        <div class="font-medium text-slate-700">Faol yuk yo'q</div>
        <a href="{{ route('app.shipments.create') }}" wire:navigate class="mt-3 inline-block text-sm text-indigo-600">➕ Yangi yuk qo'shish</a>
    </div>
    @endif
