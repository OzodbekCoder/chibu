...
@section('content')
<div class="min-h-screen flex flex-col justify-center px-6 py-12 bg-gradient-to-b from-slate-50 to-slate-100">
    <div class="max-w-sm mx-auto w-full">
        <div class="text-center mb-8">
            ...
            <h1 class="mt-4 text-2xl font-bold">CHIBU</h1>
            <p class="text-sm text-slate-500">Yuklarni boshqarish tizimi</p>
        </div>

        <form method="POST" action="{{ route('login.attempt') }}" class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 space-y-4">
            @csrf

            @if ($errors->any())
                <div class="bg-rose-50 text-rose-700 text-sm p-3 rounded-lg border border-rose-200">
                    {{ $errors->first() }}
                </div>
            @endif
            ...
            <label class="block">
                ...
                <input type="email" name="email" value="{{ old('email') }}" required autofocus inputmode="email"
                    class="mt-1 w-full rounded-xl border-slate-300 px-3 py-3 focus:border-indigo-500 focus:ring-indigo-500 border">
            </label>
